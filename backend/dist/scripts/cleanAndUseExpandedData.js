...
const defaultSettings = {
    ...
    '7x7-hard': {
        ...
        targetTimeMs: 90000,
        totalAllowedStones: 8, // 변경됨
        minWords: 7, // 최소 단어 수
        maxWords: 8, // 최대 단어 수
    }
};
// 난이도별 한국어 추가 속담 목록
const additionalKoreanProverbs = {
    '3x3-easy': [
        { text: "새벽달 보기" },
        { text: "빛 좋은 개살구" },
        { text: "서투른 변명" },
...
// 난이도별 영어 추가 속담 목록
const additionalEnglishProverbs = {
    '3x3-easy': [
...
function cleanAndUseExpandedData() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const backupData = yield ZengoProverbContent_1.default.find({}).lean();
            const timestamp = new Date().toISOString().replace(/:/g, '-');
            const backupDir = path_1.default.join(__dirname, '../backups');
            if (!fs_1.default.existsSync(backupDir)) {
                fs_1.default.mkdirSync(backupDir, { recursive: true });
            }
            fs_1.default.writeFileSync(path_1.default.join(backupDir, `zengo_backup_${timestamp}.json`), JSON.stringify(backupData, null, 2));
            console.log(`${backupData.length}개 항목 백업 완료`);
            // 2. 기존 데이터 제거
            console.log('기존 Zengo 데이터 제거 중...');
            const deleteResult = yield ZengoProverbContent_1.default.deleteMany({});
            ...
            for (const [level, proverbs] of Object.entries(koreanProverbs)) {
                ...
                for (const proverb of filteredProverbs) {
                    ...
                    const zengoData = new ZengoProverbContent_1.default({
            ...
            for (const lang of ['ko', 'en']) {
                for (const level of ['3x3-easy', '5x5-medium', '7x7-hard']) {
                    const count = yield ZengoProverbContent_1.default.countDocuments({ language: lang, level });
