...
const seedData: Partial<IZengoProverbContent>[] = [
  // --- 3x3 Easy Example --- 
  {
    level: '3x3-easy',
    language: 'ko',
...
const seedDB = async () => {
  try {
    ...
    for (const content of seedData) {
      const existing = await ZengoProverbContent.findOne({ 
      ...
      if (!existing) {
        ...
        await ZengoProverbContent.create(content as IZengoProverbContent); // Assert type for creation
