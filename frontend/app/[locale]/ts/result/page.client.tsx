...
export default function TSResultPage() {
  const t = useTranslations('ts.result');
  ...
  const getPerformanceMessage = (ppm: number) => {
    if (ppm > 2) return t('messages.excellent');
    if (ppm > 1.5) return t('messages.good');
    if (ppm > 1) return t('messages.nice');
    return t('messages.encouraging');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-blue-50 p-4">
        <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full">
          <h1 className="text-xl font-bold text-red-600 mb-4">{tCommon('error')}</h1>
          <p className="mb-6">{error || t('errors.noResults')}</p>
          <Button
            href="/ts"
            variant="default"
          >
  ...
  if (!sessionResult.bookId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-blue-50 p-4">
        <div className="bg-white rounded-xl shadow-lg p-6 max-w-md w-full">
          <h1 className="text-xl font-bold text-red-600 mb-4">{tCommon('error')}</h1>
          <p className="mb-6">{t('errors.invalidData')}</p>
          <Button
            href="/ts"
            variant="default"
          >
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-6 border border-gray-200 relative">
          ...
          <div className="relative z-10 p-6 sm:p-8 pt-12">
            ...
            <div className="text-center mb-2 mt-4">
              <h1 className="text-xl sm:text-2xl tracking-wider font-bold text-gray-800 uppercase">{t('achievement.title')}</h1>
            </div>
            ...
            <div className="text-center mb-6">
              <div className="inline-block px-3 py-1 bg-gradient-to-r from-amber-100 to-amber-50 rounded-full border border-amber-200">
                <span className="text-xs sm:text-sm font-semibold text-amber-800 uppercase tracking-wider">
                  {sessionResult.ppm > 2 ? t('achievement.labels.eliteReader') : sessionResult.ppm > 1.5 ? t('achievement.labels.advancedReader') : t('achievement.labels.dedicatedReader')}
                </span>
              </div>
            </div>
              ...
              <div className="text-center py-7 px-4 border border-gray-200 rounded-xl">
                <div className="absolute -top-4 left-0 right-0 flex justify-center">
                  <div className="px-4 py-1 bg-white rounded-full border border-gray-200 shadow-sm">
                    <span className="text-xs font-semibold text-gray-600 uppercase tracking-wider">{t('performance.readingVelocity')}</span>
                  </div>
                </div>
                <div className="flex items-center justify-center overflow-hidden">
                  ...
                  <div className="flex flex-col items-start ml-2 mt-2 sm:mt-4 lg:mt-6 flex-shrink-0">
                    <span className="text-sm text-gray-500 font-medium">{t('performance.ppmLabel')}</span>
                    <span className="text-xs text-gray-400">{t('performance.ppmUnit')}</span>
                  </div>
                </div>
                <div className="mt-2">
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div className="flex-1 border-b border-gray-200 pb-2 min-w-0">
                <p className="text-xs uppercase tracking-wider text-gray-500 mb-1">{t('details.bookTitle')}</p>
                <h2 className="text-base sm:text-lg font-semibold text-gray-800 break-words line-clamp-2">
                  {sessionResult.bookId && typeof sessionResult.bookId === 'object' 
                    ...
                    : t('details.unknownBook')}
                </h2>
                <p className="text-sm text-gray-600 mt-1 truncate">
                  {sessionResult.bookId && typeof sessionResult.bookId === 'object' 
                    ? sessionResult.bookId.author 
                    : t('details.unknownAuthor')}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-8">
              <div className="text-center border border-gray-200 rounded-lg py-3 px-2 bg-gray-50">
                <p className="text-xs uppercase tracking-wider text-gray-500 mb-1">{t('performance.totalPages')}</p>
                <p className="text-lg sm:text-xl font-semibold text-gray-800">{sessionResult.actualEndPage - sessionResult.startPage}</p>
                <p className="text-xs text-gray-500">{tCommon('pages')}</p>
              </div>
              <div className="text-center border border-gray-200 rounded-lg py-3 px-2 bg-gray-50">
                <p className="text-xs uppercase tracking-wider text-gray-500 mb-1">{t('performance.readingTime')}</p>
              ...
              <div className="text-center border border-gray-200 rounded-lg py-3 px-2 bg-gray-50">
                <p className="text-xs uppercase tracking-wider text-gray-500 mb-1">{t('details.duration')}</p>
