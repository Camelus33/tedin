 ...
 * 노트를 지식 카트에 담거나, 퀴즈 카드로 만들거나, 관련 외부 링크를 추가/관리할 수 있습니다.
...
export default function BookDetailPage() {
  ...
  const [isDeleting, setIsDeleting] = useState<boolean>(false); // 책 삭제 진행 상태
    const [flashcardFormNote, setFlashcardFormNote] = useState<PageNote | null>(null); // 퀴즈 카드 생성 폼에 전달될 노트
  const [flashcardDeckKey, setFlashcardDeckKey] = useState(0); // 퀴즈 카드 덱 강제 리렌더링을 위한 키
  const [activeTab, setActiveTab] = useState<'memo' | 'flashcard' | 'relatedLinks'>('memo'); // 현재 활성화된 탭 (메모진화, 지식연결, 퀴즈 카드)
  const [showNewFlashcardForm, setShowNewFlashcardForm] = useState(false); // 새 퀴즈 카드 수동 생성 폼 표시 여부
            ...
            {tsNotes.length === 0 ? (
              ...
              <div className={`space-y-2 sm:space-y-4 border-t ${cyberTheme.inputBorder} pt-2 sm:pt-4 max-h-[500px] sm:max-h-[600px] md:max-h-[700px] overflow-y-auto scrollbar-thin scrollbar-thumb-cyan-600 scrollbar-track-gray-800`}>
                {tsNotes.map((note) => {
                  ...
                  return (
                    <div id={`note-${note._id}`} key={note._id} className={`${cyberTheme.cardBg} p-4 sm:p-6 rounded-md border ${cyberTheme.inputBorder} transition-transform duration-300 hover:shadow-xl hover:-translate-y-0.5 cursor-pointer`}>
                      <TSNoteCard
                        note={note as TSNote}
          ...
          <section className="mt-0 bg-gray-900/80 p-2 sm:p-4 md:p-6 rounded-lg border border-cyan-500/30">
            {flashcardFormNote ? (
              ...
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-purple-400">퀴즈 카드</h2>
                <button
