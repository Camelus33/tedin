...
type TSModeStatus = 'idle' | 'warmup' | 'reading' | 'paused' | 'review' | 'completed';

interface UseTSProps {
  book?: Book;
}

export default function useTS({ book }: UseTSProps = {}) {
  const dispatch = useDispatch();
  const { currentSession, timer } = useSelector((state: RootState) => state.session);
  
  const [status, setStatus] = useState<TSModeStatus>('idle');
